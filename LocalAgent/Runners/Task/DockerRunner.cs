...
namespace LocalAgent.Runners.Tasks
{
    ...
    public class DockerRunner : StepTaskRunner
    {
        ...
        private StatusTypes RunBuild(PipelineContext context, IStageExpectation stage, IJobExpectation job, string workingDirectory)
        {
            ...
            var command = new DirectCommandBuilder("docker")
            ...
            command.ArgTokens(arguments);
            command.Arg(buildContext);

            var processInfo = command.Compile(context, stage, job, StepTask);
            GetLogger().Info($"COMMAND: '{DirectCommandBuilder.ToLogString(processInfo)}'");
        ...
        private StatusTypes RunLogin(PipelineContext context, IStageExpectation stage, IJobExpectation job)
        {
            ...
            GetLogger().Info($"COMMAND: '{DirectCommandBuilder.ToLogString(processInfo)}'");
            return RunDockerLoginProcess(processInfo, password, context);
        }

        protected virtual StatusTypes RunDockerLoginProcess(ProcessStartInfo processInfo, string password, PipelineContext context)
        {
            ...
            try
            {
                ...
                process.StandardInput.WriteLine(password ?? string.Empty);
                ...
                {
                    GetLogger().Info($"Exit Code: {process.ExitCode}");
                }
                else
                {
                    ...
                    GetLogger().Warn($"Exit Code: {process.ExitCode}");
                }
            }
            catch (Exception ex)
            {
