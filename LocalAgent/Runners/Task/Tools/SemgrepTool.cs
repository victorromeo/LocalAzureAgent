    ...
    public class SemgrepTool : LocalAgent.Runners.Tasks.Tools.PythonToolBase
    {
        ...
        public override Task<ProcessResult> RunToolAsync(string toolPath, string args, CancellationToken cancellationToken)
        {
            ...
            foreach (var cand in candidates)
            {
                ...
                {
                    if (OperatingSystem.IsWindows() && string.IsNullOrWhiteSpace(Path.GetExtension(semgrepExe)))
                    {
                        var scriptArgs = string.IsNullOrWhiteSpace(args)
                            ? $"\"{semgrepExe}\""
                            : $"\"{semgrepExe}\" {args}";

                        return base.RunPythonCommandAsync(toolPath, scriptArgs, cancellationToken);
                    }
