    ...
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            Parser.Default.ParseArguments<PipelineOptions>(args)
                .WithParsed(o =>
                {
                    if (string.IsNullOrEmpty(o.SourcePath))
                    {
                        Logger.Info("No source path provided, will attempt to infer from YAML location");
                        o.SourcePath = Path.GetDirectoryName(Path.GetFullPath(o.YamlPath));
                    }
                ...
                }).WithNotParsed(e =>
                {
                    ...
                    Logger.Error($"Expected: {appName} <yml path> <options>");
