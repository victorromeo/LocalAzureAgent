...
";
        ...
        [Fact]
        public void Deserialize_ScriptStepWithEnvMap()
        {
            var test = @"
jobs:
- job: EnvJob
  steps:
  - script: dotnet run --project ConsoleApp1/ConsoleApp1/ConsoleApp1.csproj
...
";
            ...
            var actual = PipelineContext.Deserialize(test);
            var job = (JobStandard)actual.Jobs[0];
            var step = (StepScript)job.Steps[0];

            step.Env.Should().NotBeNull();
            step.Env.Should().ContainKey("MY_SECRET");
            step.Env["MY_SECRET"].Should().Be("$(mySecret)");
        }
    }
}
