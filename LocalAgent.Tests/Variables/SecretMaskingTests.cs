...
namespace LocalAgent.Tests.Security
{
    public class SecretMaskingTests
    {
        ...
        [Fact]
        public void MaskPipelineForLogging_RedactsInlineSetVariableSecret()
        {
            var options = new PipelineOptions
            {
                AgentWorkFolder = "work",
...
";
            ...
            var masked = context.MaskPipelineForLogging(yaml);

            Assert.DoesNotContain("sample-local-secret", masked);
            Assert.Contains("##vso[task.setvariable variable=mySecret;isSecret=true]********", masked);
        }
    }
}
