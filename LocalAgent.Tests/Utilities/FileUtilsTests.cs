...
namespace LocalAgent.Tests.Utilities
{
    public class FileUtilsTests
    {
        ...
        [Fact]
        public void GuardPreventDamageToSystemFolders_ShouldAllow_SubfoldersUnderTempAndUserProfile()
        {
            var tempSubfolder = Path.Combine(Path.GetTempPath(), "LocalAgent", "guard-test");
            var userProfileSubfolder = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),
                ".LocalAgent",
                "guard-test");

            var ex1 = Record.Exception(() => FileUtils.GuardPreventDamageToSystemFolders(tempSubfolder));
            var ex2 = Record.Exception(() => FileUtils.GuardPreventDamageToSystemFolders(userProfileSubfolder));
        ...
        [Fact]
        public void GuardPreventDamageToSystemFolders_ShouldUsePathBoundaryChecks_NotRawPrefix()
        {
            string safePath;

            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                safePath = Path.Combine(Path.GetPathRoot(Environment.SystemDirectory)!, "WindowsApps", "LocalAgent");
            }
            else
            {
                safePath = "/usr/binary/localagent";
            }

            var ex = Record.Exception(() => FileUtils.GuardPreventDamageToSystemFolders(safePath));
